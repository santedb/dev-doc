...
description: >-
  ...
  specified, but with an invalid password.
---
...
1. Execute the `user.add` command with the `-r` parameter specified as an existing role, `-u` parameter specified as a non-existing username, and `-u` parameter specified as weak password.
